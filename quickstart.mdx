---
title: "Get Started with Image Transformation in 3 Steps"
...
| `rgb-channels` | `Enter channel composition:` | Any combination of `r`, `g`, `b` (e.g. `rg`, `b`, `rgb`) |
| `transparency` | `Enter transparency percentage:` | Float 0–100 (e.g. `50.0` gives 50% opacity) |

<Warning>
  Entering a value outside the expected range (for example, a non-integer for `upscale`, or an invalid string for `rgb-channels`) will cause an error. The `rgb-channels` command will re-prompt until a valid combination of `r`, `g`, and `b` is entered.
</Warning>
